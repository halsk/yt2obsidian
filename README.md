...
- YouTube動画のトランスクリプトを自動取得（YouTube APIキー不要）
- **Claude Haiku 4.5** による日本語要約を自動生成
- Obsidian raw/articles 互換のYAML frontmatter付きMarkdown出力
- 言語フォールバック（ja → en → auto）
- Obsidian安全なファイル名サニタイズ
- HTTPサーバー（モバイル対応Webフォーム + JSON API）
- **PWA + Web Share Target API** 対応（Android Chrome から YouTube 共有メニューでワンタップ投入）
- 保存後にObsidian Vault自動同期（git commit & push）
- Linux/WSL2（systemd）と macOS（launchd）両対応の常駐サービス手順

## Setup

**前提**: Node.js 20+（`brew install node` / `apt install nodejs` 等）

```bash
git clone https://github.com/halsk/yt2obsidian.git
cd yt2obsidian
npm install
# または pnpm install （pnpm-lock.yaml もあり）
```
...
| Method | Path | 説明 |
|--------|------|------|
| GET | `/` | モバイル対応Webフォーム（PWA インストール対象）|
| GET/POST | `/api/transcript` | トランスクリプト API |
| POST | `/share` | Web Share Target — Android 共有メニューから受信 |
| GET | `/manifest.json` | PWA マニフェスト |
| GET | `/sw.js` | Service Worker（PWA 必須） |
| GET | `/health` | ヘルスチェック |
...
```json
{
  ...
  "channelName": "チャンネル名",
  "filename": "動画タイトル.md",
  "outputPath": "/home/hal/workspace/obsidian/raw/articles/動画タイトル.md",
  "language": "ja",
  "hasSummary": true
}
```
...
Tailscale 経由で iPhone / Android からスマホ共有メニューを使い、ワンタップで YouTube URL を投入できる。

### Android （PWA + Web Share Target）

Android Chrome で `http://<tailscale-hostname-or-ip>:3456/` を開き、Chrome メニューから **「ホーム画面に追加」**（または「アプリをインストール」）を実行。これだけで YouTube アプリの共有メニューに **yt2obsidian** が現れ、タップ一回で URL を `/share` エンドポイントに POST → 自動保存される。追加アプリ不要。

詳細は [docs/share-from-android.md](docs/share-from-android.md) を参照。
...
デフォルトでは `~/workspace/obsidian/raw/articles/` に以下の形式で保存（`--out` で変更可）:
...
|---|---|---|
| `ANTHROPIC_API_KEY` | Anthropic API key for Claude Haiku 4.5 summary | No (summary skipped if not set) |
| `PORT` | HTTP server port (default: 3456) | No |
| `NODE_ENV` | `production` 等。systemd / launchd で設定 | No |

## Related docs

- [docs/setup-macos.md](docs/setup-macos.md) — macOS（Mac mini 等）launchd セットアップ
- [docs/share-from-android.md](docs/share-from-android.md) — Android Chrome PWA Share Target 設定詳細
